...
function loanFunction(){
  ...
  function Get_Loan_Info() {
    var loan_account = document.getElementById('sender').value;
    var account_balance;
    var url = "http://localhost/oamk_bank/api/bank/loans";
    ...
    xhttp.onreadystatechange=function() {
        if(xhttp.readyState==4 && xhttp.status==200) {
            ...
            for (i in jsonData) {
                if ( loan_account == jsonData[i].account_name ) {     
                    account_id = Number(jsonData[i].account_id);
                    account_balance = Number(jsonData[i].balance);
                    account_loan = Number(jsonData[i].loan);
                    Account_Update(account_id, account_balance, account_loan);
                }     
            }
        }
    };
  ...
  xhttp.onreadystatechange=function()
  {
    if(xhttp.readyState==4 && xhttp.status==200)
    {
      ...
      // var account_id = document.createElement('input');
      
      // document.getElementById('requestForm').appendChild(account_id);
      // account_id.setAttribute('name', 'account_id');
      // account_id.setAttribute('id', 'account_id')
      // account_id.value = document.getElementById('sender').value;
...
function request_money()
{
  ...
  var url_loan = "http://localhost/oamk_bank/index.php/api/bank/loans";
