...
function loanFunction(){
  ...
  xhttp.onreadystatechange=function()
  {
    if(xhttp.readyState==4 && xhttp.status==200)
    {
      ...
      for(x in jsonData)
      {
        ...
        option_sender.setAttribute('value', jsonData[x].account_id);
 ...
 function right_balance(){
  ...
  document.getElementById('loading').innerHTML=account_id.value;

  var xhttp = new XMLHttpRequest();
  var url_loan = "http://localhost/oamk_bank/index.php/api/bank/accounts/accountid/"+ account_id.value;
  xhttp.open('GET', url_loan, true);
  debugger
  xhttp.onreadystatechange=function()
  {
    if(xhttp.readyState==4 && xhttp.status==200)
    {
      ...
      var acc_balance = document.getElementsByName('Balance');
      document.getElementById('loading').innerHTML = jsonData[0].Balance;
    }
  };
  xhttp.send();
}
